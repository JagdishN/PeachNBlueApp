...
**Deliberately not done — flagging rather than silently building or skipping**: the ₹350 `AdditionalCharge` is **not wired into invoice generation, the ledger, or any payment-collection flow** — it's recorded and visible (via the replacement count), but nothing currently adds it to what a customer is asked to pay. CLAUDE.md's schema note calls it "a distinct line item (not a garment charge)," which implies it should eventually show up on a bill somewhere — that's a separate, larger task (touching `invoiceService.ts`/`orderService.ts`'s payable-amount calculation) that wasn't asked for here and would need its own scoping (e.g. does it attach to the *next* order, get billed standalone, wait for the monthly statement for ledger customers?).

Tests: `src/__tests__/customerController.test.ts`'s new "bag-issued"/"bag-replacement"/"bag-replacements" blocks (idempotency, 404s, the bagIssued-required-first validation, the fixed ₹350 amount, role gates).

---

## Payment marking — real gap, discovered during the staff-scoped visibility ground-truth check, not yet built

`Order.paymentStatus` exists on the schema (`String @default("pending")`) but **no endpoint anywhere updates it** — confirmed via a direct grep across `src/controllers` for `paymentStatus`/payment-marking logic, zero matches. This directly leaves half of this file's own "Order workflow" step 7 unimplemented: "At delivery, the executive verifies payment status and marks the order both **Delivered** and **Payment Received**" — the "Delivered" half is real (`PATCH /orders/:id/status`), the "Payment Received" half isn't wired to anything. `updateStatus`'s existing `billingMode` branch (see "Monthly billing" above) only posts a ledger charge for monthly-billing customers on delivery; it doesn't touch `paymentStatus`, and there's no equivalent path for daily customers to actually get marked paid.

Needs: a way to record which of the four confirmed payment modes (Cash, UPI, Net Banking, Credit Card) was used and flip `paymentStatus` to `'paid'`, most likely bundled into the same delivery action rather than a separate screen (staff is standing at the door either way). Not built here — flagged, not designed, since it wasn't part of this round's ask and touches the same delivery transition the billingMode branch already owns.
