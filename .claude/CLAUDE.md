...
**One data anomaly transcribed as-is, not silently corrected — still needs client confirmation:**
...
**Genuinely open, needs your confirmation (not the client's) before building**: is "discounts/coupons" meant as just this on/off toggle for the existing persistent per-flat percentage, or a separate redeemable coupon-code system? Proceeded with the toggle-only interpretation below; flag if coupon codes are actually wanted — that's a different, larger feature.
